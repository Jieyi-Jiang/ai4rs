...
custom_imports = dict(
    imports=['projects.OrientedFormer_curriculum.orientedformer'], allow_failed_imports=False)
...
model = dict(
    ...
    dn_cfg=dict(  # TODO: Move to model.train_cfg ?
        ...
        dynamic_schedule=[
            dict(start=0, end=11, ratio=0.2),
            dict(start=12, end=23, ratio=0.5),
            dict(start=24, end=35, ratio=1.0),],
    ...
    roi_head=dict(
        ...
        bbox_head=[
            dict(
                ...
                bbox_coder=dict(type='DeltaXYWHTRBBoxCoder')) for stage_idx in range(num_stages)
        ]),
