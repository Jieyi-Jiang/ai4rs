...
from projects.rotated_dino.rotated_dino import RotatedDINO, RotatedMultiScaleDeformableAttention
from .rtdetr_layers import RTDETRHybridEncoder
...
class RotatedRTDETR(RotatedDINO):
    ...
    def gen_proposals(
        ...
        for lvl, HW in enumerate(spatial_shapes):
            ...
            rad = torch.ones_like(grid_x).expand(batch_size, -1,
                                                 -1).unsqueeze(-1) * 0.5
            proposal = torch.cat((grid, wh, rad), -1).view(batch_size, -1, 5)
