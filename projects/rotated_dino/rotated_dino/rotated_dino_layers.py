...
class RotatedDinoTransformerDecoder(DinoTransformerDecoder):
    """Transformer decoder of RotatedDino."""
    ...
    def generate_dn_bbox_query(self, gt_bboxes: Tensor, num_groups: int) -> Tensor:
        ...
        Args:
            gt_bboxes (Tensor): The concatenated gt bboxes of all samples
                in the batch, has shape (num_target_total, 5) with the last
                dimension arranged as (cx, cy, w, h, angle) where
                `num_target_total = sum(num_target_list)`.
            num_groups (int): The number of denoising query groups.

        Returns:
            Tensor: The output noisy bboxes, which are embedded by normalized
            (cx, cy, w, h, angle) format bboxes going through inverse_sigmoid, has
            shape (num_noisy_targets, 5) with the last dimension arranged as
        ...
        # Prepare noise tensor (same shape as gt_bboxes_expand)
        noise = torch.zeros_like(gt_bboxes_expand)
        ...
        if self.noise_mode in ('only_xyxy', 'only_xywh', 'all_xyxya'):
            ...
            rand_part = torch.rand_like(gt_bboxes_expand[:, :4])  # [0, 1)
        ...
        if self.noise_mode in ('only_angle', 'all_xyxya'):
            # determine the sign of each element in the random part of the added
            # noise to be positive or negative randomly.
            rand_sign_angle = torch.randint_like(
                gt_bboxes_expand[:, 4], low=0, high=2,
                dtype=torch.float32) * 2.0 - 1.0  # [low, high), 1 or -1, randomly

            # calculate the random part of the added noise
            rand_part_angle = torch.rand_like(gt_bboxes_expand[:, 4])  # [0, 1)
            rand_part_angle[negative_idx] += 1.0  # pos: [0,1), neg: [1,2)
            rand_part_angle *= rand_sign_angle  # pos: (-1,1), neg: (-2,-1] ∪ [1,2)
        ...
        if self.noise_mode in ('only_xyxy', 'only_angle', 'all_xyxya'):
            noisy_bboxes = torch.cat(
                [bbox_xyxy_to_cxcywh(bbox_cxcywh_to_xyxy(gt_bboxes_expand[:, :4]) + noise[:, :4]),
                 gt_bboxes_expand[:, 4:] + noise[:, 4:]], dim=-1)
            noisy_bboxes = noisy_bboxes.clamp(min=0.0, max=1.0)
        elif self.noise_mode in ('only_xywh'):
            noisy_bboxes = gt_bboxes_expand + noise
            noisy_bboxes = noisy_bboxes.clamp(min=0.0, max=1.0)
        else:
            raise NotImplementedError

        dn_bbox_query = inverse_sigmoid(noisy_bboxes, eps=1e-3)
