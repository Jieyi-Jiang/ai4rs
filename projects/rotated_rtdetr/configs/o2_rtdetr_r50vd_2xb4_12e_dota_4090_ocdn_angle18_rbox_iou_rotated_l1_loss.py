...
from math import pi
from projects.rotated_rtdetr.rotated_rtdetr.rotated_l1_loss import RotatedL1Loss
...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_cls=dict(
            ...
            varifocal_loss_iou_type='rbox_iou', # hbox_iou, rbox_iou, prob_iou
            use_sigmoid=True,
            alpha=0.75,
            gamma=2.0,
            iou_weighted=True,
            loss_weight=1.0),
        loss_bbox=dict(type=RotatedL1Loss, loss_weight=5.0),
        loss_iou=dict(
            type=GDLoss,
            loss_type='kld',
            fun='log1p',
    ...
    dn_cfg=dict(  # TODO: Move to model.train_cfg ?
        ...
        box_noise_scale=1.0,
        angle_noise_scale=pi/18.0,
        angle_cfg=angle_cfg,
        angle_factor=angle_factor,
        noise_mode='all_xyxya',  # 'only_xyxy', 'only_angle', 'only_xywh', 'all_xyxya'
...
train_dataloader.update(batch_size=6, num_workers=3)
val_dataloader.update(batch_size=6, num_workers=3)
test_dataloader.update(batch_size=6, num_workers=3)
